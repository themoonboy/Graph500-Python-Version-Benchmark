...
"""
from kronecker_generator_memmap import *
from kernel_1_memmap import *
from kernel_2_memmap import *
...
for k in range(NBFS):
    t_start = time.time()
    parent = kernel_2_custom(G, search_key[k])
    t_end = time.time()
    ...
    for node in parent:
        if node>=0:
            kernel_2_nedge[k] += len(np.where(G[:,int(node)]>0)[0])
    kernel_2_nedge[k] /= 2
    # kernel_3 ignored
