def kronecker_generator(SCALE, edgefactor):
    ...
    # Create index arrays.
    #ijw = np.ones((3, M))
    ijw = np.memmap('temp/ijw', dtype='float32' , mode='w+', shape=(3, M))
    ijw += 1
    ...
    ii_bit = np.memmap('temp/ii_bit', dtype='float32' , mode='w+', shape=(1, M))
    jj_bit = np.memmap('temp/jj_bit', dtype='float32' , mode='w+', shape=(1, M))
